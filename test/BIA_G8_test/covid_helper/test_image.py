...
def test_apply():
    c1 = covid_dataset.CovidImage.from_np_array(stride, 0, covid_dataset.default_decode(0))
    c2 = c1.apply(lambda img: skitrans.rotate(img, 90))
    assert c2.label == 0
    assert c2.label_str == covid_dataset.default_decode(0)
    c3 = c2.apply(lambda img: skimage.img_as_int(skitrans.rotate(img, 270)))
    assert np.array_equiv(c1.np_array, c3.np_array)


def test_save_load():
    c1 = covid_dataset.CovidImage.from_np_array(stride, 100, covid_dataset.default_decode(100))
    assert c1.label == 100
    ...
    c2 = covid_dataset.CovidImage.from_file(tmp_save_path_npy, 100, covid_dataset.default_decode(100))
    ...
    c3 = covid_dataset.CovidImage.from_file(tmp_save_path_tiff, 100, covid_dataset.default_decode(100))
